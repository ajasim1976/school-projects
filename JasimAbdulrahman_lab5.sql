...
FROM Currency
WHERE currency_id IN (SELECT currency_accepted_id 
					  FROM Store_location
					  WHERE store_name = 'Cancun Extension');

SELECT price_in_us_dollars * 16.75 AS price_in_mexican_peso
FROM Product 
WHERE product_name = 'Flowing Skirt'

-- 5-b.
CREATE TABLE MyDecimal (
mydecimal_num DECIMAL(8,2)
)
-- 5 - c
INSERT INTO MyDecimal (mydecimal_num)
VALUES (1234356.65)

SELECT * FROM MyDecimal 
...
SELECT p.product_name As 'Product_Name',
	   (SELECT FORMAT(p.price_in_us_dollars *
									(SELECT c.us_dollars_to_currency_ratio 
									 FROM Currency c
									 WHERE c.currency_name = 'Mexican Peso'),'$.00')
		) AS 'Mexican_Peso'
FROM Product p
WHERE product_name = 'Flowing Skirt';
...
SELECT  p.product_name As 'Product Name',
	   (SELECT FORMAT(p.price_in_us_dollars *
									(SELECT c.us_dollars_to_currency_ratio 
									 FROM Currency c
									 WHERE c.currency_name = 'Canadian Dollar'),'$.00')
		) AS 'Canadian Dollar'

FROM Product p
WHERE p.price_in_us_dollars * (SELECT us_dollars_to_currency_ratio
							   FROM Currency
							   WHERE currency_name = 'Canadian Dollar') < 150 
OR p.price_in_us_dollars * (SELECT us_dollars_to_currency_ratio
							   FROM Currency
							   WHERE currency_name = 'Canadian Dollar') > 300;


-- 20 
SELECT Product.product_name AS 'Product Name', Sizes.size_option AS 'Size Options'
FROM Product
...
-- 22 
SELECT PROD.product_id, PROD.product_name, Sizes.size_option AS 'Size Options'
FROM (SELECT Product.product_id, product_name
	  FROM Product
	  JOIN Sells ON Product.product_id = Sells.product_id
	  GROUP BY Product.product_id, Product.product_name
      HAVING COUNT(Sells.store_location_id) = 5) PROD

JOIN Available_in ON PROD.product_id = Available_in.product_id 
JOIN Sizes ON Sizes.sizes_id = Available_in.sizes_id
